...
object HostMap {
  ...
  def main(args:Array[String]){
    ...
    LOG.info("Total memory: " + totalMemGB + "GB")

    load(new File("D:/nlp/data/uriMap.tsv"))

    val freeMemGBafter : Double = Runtime.getRuntime.freeMemory / 1073741824.0
    val maxMemGBafter : Double = Runtime.getRuntime.maxMemory / 1073741824.0
    val totalMemGBafter :Double =  Runtime.getRuntime.totalMemory / 1073741824.0
    LOG.info("Available memory after loading: "+freeMemGBafter+"GB")
    LOG.info("Max memory  after loading: "+ maxMemGBafter +"GB")
    LOG.info("Total memory  after loading: " + totalMemGBafter + "GB")
  }
}

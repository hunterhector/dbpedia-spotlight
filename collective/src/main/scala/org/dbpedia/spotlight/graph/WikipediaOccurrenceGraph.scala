...
/**
 * Create a Integer list to represent an occurrence graph (You can use GraphUtils to build)
 */
class WikipediaOccurrenceGraph {
  private val LOG = LogFactory.getLog(this.getClass)

  /**
   * core function that parse the occurrences file into integer list
   * @param occsFile Occurrence file generated by ExtractOccsFromWikipedia
   * @param hostMap Hostmap that stores the uri to integer mapping
   * @param tripleListFile The output file store each arc per line as a tab separated triple
   */
  def buildTripleList(occsFile: File,  hostMap:CompactHashMap[String,Int], tripleListFile: File) = {
    ...
    LOG.info(String.format("Integer List File saved at %s", tripleListFile.getName))
  }

  // well, now don't have specific implementation to give preferences to particular resource pair
  // might be no reason to do so anyway

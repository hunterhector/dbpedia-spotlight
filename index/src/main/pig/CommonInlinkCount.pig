-- considering the whole doc is too inefficient and the result is huge. maybe useless

-- This script is to count the Inlink from entities from the TSV file generated by ExtractOccsFromWikipedia.
-- The output would be stored into JSON like format.
-- The script is similar to CoocurrenceCount.pig because of they are actually doing the similar thing, differ by the doc level and whether to count multiple entires in one page
-- Two scripts are created to make it simple to call.
...
SET job.name CommonInLinkCount;
...
entityPairs = FOREACH coOccsById {
	...
	generate flatten(uniq_uri) AS (ent1) ,flatten(uniq_uri) AS (ent2);
};
...
JSONAdjLists = FOREACH adjLists GENERATE group,cnt.(ent2,count);

-- Write out
-- Consider use JSONStorage
STORE JSONAdjLists INTO '$outDir/commonInlinkCount.tsv' USING PigStorage();